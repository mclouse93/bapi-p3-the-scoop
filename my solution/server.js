...
function createComment(url, request) {
  const requestComment = request.body && request.body.article && request.body.comment;
  const articleValid = request.body && database.articles[request.body.articleId];
  const commentValid = request.body && request.body.comment;
  const response = {};

  if (requestComment && articleValid && commentValid &&
